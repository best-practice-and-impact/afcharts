...
This line chart uses the afcharts theme. There are pale grey gridlines extending from the y-axis, and there is a thicker dark blue line representing the data.
...
This bar chart uses the afcharts theme, and shows the populations of the five most populous countries in the Americas. Each bar is dark blue and labelled by country underneath. All text is black in a sans serif font. Pale grey gridlines extend out from the y-axis.

A bar chart can sometimes be easier to interpret with horizontal bars. This can also be a good option if your bar labels are long and difficult to display on the x-axis. To produce a horizontal bar chart, first swap the variables defined for x and y in `aes()`. Then, make a few tweaks to `theme_af()` by drawing gridlines for the x-axis only by setting the `grid` argument, and drawing an axis line for the y-axis only by setting the `axis` argument. We can also hide the x and y-axis titles by setting the `axis_title` argument. This replaces the need to use the `labs` function, with x and y set to NULL.
...
This bar chart uses the afcharts theme and displays the populations of the five most populous countries in the Americas. The country names are displayed on the y-axis, with the bars extending from left to right. Each bar is dark blue, and pale grey gridlines extend up from the x-axis.
...
Caution should be taken when producing stacked bar charts. They can quickly become difficult to interpret if plotting non part-to-whole data, or if plotting more than two categories per stack. First and last categories in the stack will always be easier to compare across bars than those in the middle. Think carefully about the story you are trying to tell with your chart.
...
This histogram uses the afcharts theme, and shows the distribution of life expectancy by number of countries. There are pale grey gridlines extending out from the y-axis. The bars are dark blue with white space between each.
...
It is best practice to use commas to separate thousands. Use scales::label_comma() to add commas to axis labels.
...
<div class="chart-subtitle">Population growth in billions by continent, 1952 to 2007</div>
...
**Note**: Interactive charts may not meet all accessibility requirements. We advise you to test any interactive charts with keyboard and screen reader users, to ensure they work correctly.
...
This is an interactive bar chart using the afcharts theme, showing the populations of the five most populous countries in the Americas. The bars are dark blue. A tooltip appears when hovering over each bar with the cursor, which shows a matching dark blue box with the country and population displayed in white text. Click and drag on the chart using the cursor to zoom in on the selected area. Click the 'home' icon in the top right corner to reset the view.

## Choropleth charts
```{r choropleth-charts, eval = FALSE}
# Read in a geospatial dataset of pay data combined with local authority boundaries
lad_data <- read_rds("geospatial_data.rds")

# Filter Northern Ireland missing data and discretise pay data
lad_data <- lad_data |>
  filter(!str_detect(LAD24CD, "N")) |>
  mutate(
    discrete_pay = cut_interval(
      median_ann_pay,
      n = 5,
      right = FALSE,
      labels = c(
...
# Create map for England, Scotland, Wales, excluding Shetland Islands
base_map <- lad_data |>
  ggplot() +
  geom_sf(aes(fill = discrete_pay), colour = "grey50") +
  coord_sf(
    xlim = c(-7.9, 1.9),
    ylim = c(49.8, 59.4),
    expand = FALSE
...
# Create inset for the Shetland Islands
shetland <- base_map +
  coord_sf(
    xlim = c(-2.25, -0.6),
    ylim = c(59.45, 60.9),
...
full_map
```
<div class="chart-title">Gross Annual Pay by Local Authority</div>
<div class="chart-subtitle">Median Gross Annual Pay in England, Scotland and Wales by Local Authority (£), 2024</div>
