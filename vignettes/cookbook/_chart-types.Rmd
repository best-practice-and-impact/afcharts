...
A bar chart can sometimes be easier to interpret with horizontal bars. This can also be a good option if your bar labels are long and difficult to display on the x axis. To produce a horizontal bar chart, swap the variables defined for x and y in `aes()` and make a few tweaks to `theme_af()`; draw grid lines for the x axis only by setting the `grid` argument, and draw an axis line for the y axis only by setting the `axis` argument. We can also hide the x and y axis titles by setting the `axis_title` argument. This replaces the need to use the `labs` function, with x and y set to NULL.
...
Please note, interactive charts may not meet all accessibility requirements and we advise you to test any interactive charts with users to ensure they work correctly.
...
# Filter Northern Ireland because of no data
lad_data <- lad_data %>%
  filter(!str_detect(LAD24CD, "N"))

# Discretise pay data into bins
breaks <- c(0, 25, 30, 35, 40, 50) * 1000

lad_data$discrete_pay <- cut(
  lad_data$median_ann_pay,
  breaks = breaks,
  dig.lab = 5
)
